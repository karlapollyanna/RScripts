...
##### Checking packages and installing/loading when necessary (I found this function at Stackoverflow. Is pretty usefull)
...
Required_Packages = c("corrplot", "PerformanceAnalytics", "scatterplot3d")  # Choose packages 
...
rows <- nrow(PCA)				# Read the number of rows (will be used in DF / DF2 - data frames


##### Defining Variables to be uses

d1 <- PCA[,2:9]
pr=prcomp(d1, cor=TRUE, scores=TRUE, scale=TRUE)	# Scale when variables have diff scales such as 1:10 and 200000:500000
...
d2 <- PCA[,c(2,6,9)] # Complet c() with specific collumns decided previously
